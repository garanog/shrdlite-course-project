...
\maketitle
\begin{abstract}
The introduction of Autonomous Vehicles promises a variety of significant advantages over conventional cars. Those advantages include increased safety, decreased individual transportation cost, mobility for everyone and a potential decrease in energy consumption and emissions. 

The release of autonomous vehicles in the near future has been announced by the majority of car manufacturers. We give an overview over how current state of the art AVs, which are not yet commercially available, use sensors and machine learning learning approaches for autonomous driving.
...
\end{itemize}
For a more functional perspective, sensors can be organized into categories defined by its use cases for AVs:
\\\\\textbf{Know where you are going}:
\begin{itemize}
\item \textbf{GPS}: Allows the AV to determine its position in absolute terms. In optimal conditions it has good accuracy and precision, but it cannot be constantly relied on due to intermittent coverage caused by tunnels, canyons, buildings, radio interference, etc. \cite{SchweberMouser}
...
For the correct operation of the AV other sensors are needed, which deal with the internal state of the vehicle, such as: Battery \& fuel level sensors, engine temperature, oil pressure, etc.
