...
module Parser {
    ...
    export function parse(input: string): ParseResult[] {
        let nearleyParser = new nearley.Parser(grammar.ParserRules, grammar.ParserStart);
        let parsestr = input.toLowerCase().replace(/\W/g, "");
        try {
            let results: Command[] = nearleyParser.feed(parsestr).results;
        } catch (err) {
            if ("offset" in err) {
...
declare module "grammar" {
    export let ParserRules: { [s: string]: any };
...
if (typeof require !== "undefined") {
    ...
    let nearley = require("./lib/nearley.js");
